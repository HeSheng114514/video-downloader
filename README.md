...
* 🍪 **登录态支持**：手动粘贴 Cookie / cookies.txt / Firefox 读取（绕过 App-Bound 加密）
...
* **v1.2.0**
  * **界面全面改版**：卡片式布局（链接输入 / 下载选项 / 任务列表 / 运行日志四张卡片），
    统一 1px 描边与 8-16px 间距，层次更清晰
  * 顶栏：品牌色标题 + 版本徽章 + 右侧工具栏，底部 2px 强调色分隔线
  * 新增：顶栏 **「🌙 深色 / ☀ 浅色」一键切换主题**（切换时重建界面，任务/输入/日志不丢失）
  * 任务列表：加宽进度列、斑马纹、状态着色（绿=完成，红=失败，橙=暂停，蓝=进行中）
  * 进度列更简洁：完成显示 `✔ 100%`，未开始显示 `— / 待下载`，进行中才显示进度条
  * 状态栏：新增运行环境指示灯（绿点=就绪，红点=缺依赖）
  * 按钮分三档（主操作实心 / 次操作浅底描边 / 工具栏无边框），语义一致
  * 四个对话框（设置 / Cookie 助手 / 手动填写 Cookie / 运行环境）统一配色，
    深色模式下不再出现白底突兀的文本框
  * 修复：深色下设置页标签页露出 **白色 3D 边框**（clam 的 `TNotebook.Tab`
    默认边框色未覆盖，需显式指定 `lightcolor`/`darkcolor`/`bordercolor`）
  * 修复：深色下**复选框方框发白、勾选标记几乎看不见**。经对照实验确认 clam 的正确选项是
    `indicatorbackground`（方框底色）/ `indicatorforeground`（勾的颜色）/
    `upperbordercolor`·`lowerbordercolor`（边框色）—— 现在未选中为描边空框，
    选中为强调色填充 + 对比色勾，浅色深色都清晰
  * 修复：设置对话框过高时可能超出屏幕底部（现限制在屏幕可用区域内）
* **v1.1.1**
  * 修复：列表里已存在同一视频时，再次点「开始下载」不再毫无反应并清空输入框
  * 新增：**设置 → 下载 → 「重复视频处理」开关**，可在两种行为间切换：
    * **覆盖重新下载**（默认）—— 强制重新下载并替换原文件
    * **跳过重复** —— 忽略已存在的视频，不重新下载（输入框会保留，不会被误清空）
  * 修复：「🔍 解析」按钮存在同样的「清空输入框但没任何动作」问题
  * 修复：已完成的任务右键「重新下载」时，因同名文件已存在会被 yt-dlp 跳过（表现为点了没反应），
    现在会强制覆盖
  * 新增：按「视频」查重 —— 同一视频的不同链接形式（BV 号带参数、`b23.tv` 短链、
    `youtu.be` 短链、抖音/快手短链等）会识别为同一个任务，不再重复占位
  * 新增：正在下载中的任务被重复提交时，保留输入框并提示，不再误清空
  * 优化：连续粘贴多个链接不再粘连成一条无效 URL
* **v1.1.0**
